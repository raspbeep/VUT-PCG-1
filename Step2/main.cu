...
int main(int argc, char** argv) {
  ...
  CUDA_CALL(cudaMemcpy(dParticles[1].position_x, hParticles.position_x, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].position_y, hParticles.position_y, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].position_z, hParticles.position_z, allocSize,
                       cudaMemcpyHostToDevice));

  CUDA_CALL(cudaMemcpy(dParticles[1].velocity_x, hParticles.velocity_x, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].velocity_y, hParticles.velocity_y, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].velocity_z, hParticles.velocity_z, allocSize,
                       cudaMemcpyHostToDevice));
