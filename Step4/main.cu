...
int main(int argc, char** argv) {
  ...
  /********************************************************************************************************************/
  CUDA_CALL(cudaMemcpy(dParticles[0].position_x, hParticles.position_x, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[0].position_y, hParticles.position_y, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[0].position_z, hParticles.position_z, allocSize,
                       cudaMemcpyHostToDevice));

  CUDA_CALL(cudaMemcpy(dParticles[0].velocity_x, hParticles.velocity_x, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[0].velocity_y, hParticles.velocity_y, allocSize,
                       cudaMemcpyHostToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[0].velocity_z, hParticles.velocity_z, allocSize,
                       cudaMemcpyHostToDevice));

  CUDA_CALL(cudaMemcpy(dParticles[0].mass, hParticles.mass, allocSize, cudaMemcpyHostToDevice));

  // can be copied in device
  CUDA_CALL(cudaMemcpy(dParticles[1].position_x, dParticles[0].position_x, allocSize,
                       cudaMemcpyDeviceToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].position_y, dParticles[0].position_y, allocSize,
                       cudaMemcpyDeviceToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].position_z, dParticles[0].position_z, allocSize,
                       cudaMemcpyDeviceToDevice));

  CUDA_CALL(cudaMemcpy(dParticles[1].velocity_x, dParticles[0].velocity_x, allocSize,
                       cudaMemcpyDeviceToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].velocity_y, dParticles[0].velocity_y, allocSize,
                       cudaMemcpyDeviceToDevice));
  CUDA_CALL(cudaMemcpy(dParticles[1].velocity_z, dParticles[0].velocity_z, allocSize,
                       cudaMemcpyDeviceToDevice));

  CUDA_CALL(
      cudaMemcpy(dParticles[1].mass, dParticles[0].mass, allocSize, cudaMemcpyDeviceToDevice));
  ...
  /********************************************************************************************************************/
  /*                           Declare and create necessary CUDA streams and events */
  /********************************************************************************************************************/
  ...
  const std::size_t redSharedMemSize =
      (simBlockDim /* warpSize*/) * sizeof(float) * 4;  // you can use warpSize variable
  ...
  auto getRecordNum = [writeFreq](unsigned s) -> unsigned { return s / writeFreq; };

  // Start measurement
  const auto start = std::chrono::steady_clock::now();

  /********************************************************************************************************************/
  /*            Edit the loop to work asynchronously and overlap computation with data
   * transfers.               */
  ...
  for (unsigned s = 0u; s < steps; ++s) {
    ...
    /******************************************************************************************************************/
    /*                GPU kernels invocation with correctly set dynamic memory size and stream
     */
    /******************************************************************************************************************/
    calculateVelocity<<<simGridDim, simBlockDim, sharedMemSize, stream_velocity>>>(
        dParticles[srcIdx], dParticles[dstIdx], N, dt);
    ...
    if (shouldWrite(s)) {
      ...
      CUDA_CALL(cudaMemcpyAsync(hParticles.position_x, dParticles[srcIdx].position_x, allocSize,
                                cudaMemcpyDeviceToHost, stream_write));
      CUDA_CALL(cudaMemcpyAsync(hParticles.position_y, dParticles[srcIdx].position_y, allocSize,
                                cudaMemcpyDeviceToHost, stream_write));
      CUDA_CALL(cudaMemcpyAsync(hParticles.position_z, dParticles[srcIdx].position_z, allocSize,
                                cudaMemcpyDeviceToHost, stream_write));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velocity_x, dParticles[srcIdx].velocity_x, allocSize,
                                cudaMemcpyDeviceToHost, stream_write));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velocity_y, dParticles[srcIdx].velocity_y, allocSize,
                                cudaMemcpyDeviceToHost, stream_write));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velocity_z, dParticles[srcIdx].velocity_z, allocSize,
                                cudaMemcpyDeviceToHost, stream_write));
      ...
      centerOfMass<<<redGridDim, redBlockDim, redSharedMemSize, stream_com>>>(
          dParticles[srcIdx], dCenterOfMass, dLock, N);
      ...
      CUDA_CALL(cudaMemcpyAsync(hCenterOfMass, dCenterOfMass, sizeof(float) * 4,
                                cudaMemcpyDeviceToHost, stream_write));
  ...
  /********************************************************************************************************************/
  /*                          Invocation of center of mass kernel, do not forget to add */
  ...
  CUDA_CALL(cudaStreamWaitEvent(stream_com, event_com_memset_done));
  centerOfMass<<<redGridDim, redBlockDim, redSharedMemSize, stream_com>>>(dParticles[resIdx], dCenterOfMass,
                                                              dLock, N);
  ...
  /********************************************************************************************************************/
  CUDA_CALL(cudaMemcpy(hParticles.position_x, dParticles[resIdx].position_x, allocSize,
                       cudaMemcpyDeviceToHost));
  CUDA_CALL(cudaMemcpy(hParticles.position_y, dParticles[resIdx].position_y, allocSize,
                       cudaMemcpyDeviceToHost));
  CUDA_CALL(cudaMemcpy(hParticles.position_z, dParticles[resIdx].position_z, allocSize,
                       cudaMemcpyDeviceToHost));

  CUDA_CALL(cudaMemcpy(hParticles.velocity_x, dParticles[resIdx].velocity_x, allocSize,
                       cudaMemcpyDeviceToHost));
  CUDA_CALL(cudaMemcpy(hParticles.velocity_y, dParticles[resIdx].velocity_y, allocSize,
                       cudaMemcpyDeviceToHost));
  CUDA_CALL(cudaMemcpy(hParticles.velocity_z, dParticles[resIdx].velocity_z, allocSize,
                       cudaMemcpyDeviceToHost));

  CUDA_CALL(
      cudaMemcpy(hParticles.mass, dParticles[resIdx].mass, allocSize, cudaMemcpyDeviceToHost));
  ...
  std::printf("Center of mass on GPU: %f, %f, %f, %f\n", hCenterOfMass->x, hCenterOfMass->y,
              hCenterOfMass->z, hCenterOfMass->w);
